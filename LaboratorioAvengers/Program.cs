...
class LaboratorioAvengers // Clase principal del programa
{
    ...
    // Copiar un archivo: Copia el archivo "inventos.txt" a una carpeta llamada "Backup"
    static void CopiarArchivo(string origen, string destino)
    {
        try
        {
            if (File.Exists(origen)) // Verificar si el archivo de origen existe
            {
                ...
                if (!Directory.Exists(carpetaBackup)) // Si la carpeta no existe, crearla
                {
                    Directory.CreateDirectory(carpetaBackup);
                }

                if (!File.Exists(destino)) // Verificar si el archivo ya existe en Backup
                {
                    File.Copy(origen, destino); // Copiar el archivo si no existe en destino
                    Console.WriteLine("\nArchivo copiado a Backup exitosamente.");
                }
                else
                {
                    Console.WriteLine("\nEl archivo ya existe en Backup, no se ha copiado nuevamente.");
                }
            }
            else
            {
    ...
    // Mover un archivo: Mueve el archivo "inventos.txt" a una carpeta llamada "ArchivosClasificados"
    ...
    // Crear una carpeta: Crea una nueva carpeta llamada "ProyectosSecretos"
    ...
    // Verificar si existe un archivo: Antes de realizar cualquier operación, verifica si el archivo "inventos.txt" existe
    ...
    // Eliminar un archivo: Elimina el archivo "inventos.txt" después de hacer una copia de seguridad
    ...
    // Listar archivos: Muestra en la consola todos los archivos dentro de la carpeta "LaboratorioAvengers"
